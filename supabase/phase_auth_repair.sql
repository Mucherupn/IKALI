-- Phase auth repair: stabilize auth/profile onboarding + pro application flow
-- Safe, idempotent migration (does not drop user data).
...
create table if not exists public.profiles (
  ...
  latitude numeric,
  longitude numeric,
  pro_application_status text not null default 'not_applied',
...
alter table public.profiles
  ...
  add column if not exists pro_application_status text,
...
set
  ...
  pro_application_status = coalesce(pro_application_status, 'not_applied')
where role is null or pro_application_status is null;

alter table public.profiles
  alter column role set default 'customer',
  alter column role set not null,
  alter column pro_application_status set default 'not_applied',
  alter column pro_application_status set not null;
...
create policy if not exists "Pro applications are updatable by owner while pending"
  on public.pro_applications
  for update
  using (auth.uid() = user_id and status in ('pending', 'needs_more_info'))
  with check (auth.uid() = user_id);
...
-- TODO: add explicit admin RLS policies for profiles/pro_applications/providers once admin auth helper standardization is complete.
